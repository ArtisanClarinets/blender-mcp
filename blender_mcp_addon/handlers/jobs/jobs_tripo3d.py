...
def get_status() -> Dict[str, Any]:
    ...
    logger.info("Getting Tripo3D status", enabled=enabled)
    return {
        "enabled": enabled,
...
def create_job(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not payload.get("text_prompt") and not payload.get("image_url"):
        raise BlenderMCPError("text_prompt or image_url is required")

    job = create_job_record(_jobs, "tripo3d", payload, status="pending")
    logger.info("Created Tripo3D job", job_id=job["job_id"])
    return {
        "job_id": job["job_id"],
...
def poll_job_status(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not task_id:
        raise BlenderMCPError("task_id is required")

    job = _jobs.get(task_id)
    if job is None:
        raise BlenderMCPError(f"Job not found: {task_id}")
    ...
    logger.info("Polled Tripo3D job status", job_id=job["job_id"], status=job["status"])
    return response
...
def import_model(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not model_url:
        raise BlenderMCPError("model_url is required")

    logger.info("Importing Tripo3D model", name=name, model_url=model_url)
...
def import_job_result(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("Importing Tripo3D job result", job_id=job_id, name=name)
    return build_import_response(
        "tripo3d",
        name,
        job_id=job_id,
