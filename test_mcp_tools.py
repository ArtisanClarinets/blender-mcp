...
Tests all available tool functions through the MCP protocol.
"""
...
def test_scene_observation(sock):
    ...
    response = send_command(sock, "observe_scene", {"max_objects": 10})
...
def test_rendering(sock):
    ...
    response = send_command(
        sock,
        "render_preview",
        {"resolution": [256, 256], "output_path": "//test_preview.png"},
    )
    assert response.get("ok") == True, f"render_preview failed: {response}"
